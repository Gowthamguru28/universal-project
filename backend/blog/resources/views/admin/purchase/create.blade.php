		...
		<div class="card-body p-3">
			...
			<div class="col-md-6 col-lg-4 col-xl-3">
				<div class="form-group">
					...
					<input type="text" placeholder="" name="po_number" value="2" readonly class="form-control">
			...
			<div class="col-md-6 col-lg-4 col-xl-3">
				<div class="form-group">
					...
					<select name="vendor_name" id="vendor_id" class="form-control">
...
<table id="purchase_table" class="order-table controls table table-light table-bordered table-striped table-hover">
	...
	@if(old('brand_id') == '')
	<tr class="entry">
		<td><select id="new_brand" name="brand_id[]" class="brand-creation brand-create" placeholder="Brand Name" onchange="brand_change(this);" required>
		...
		<td>
			<select id="new_item"  name="item_id[]" class="brand-creation item-create" placeholder="Item Name" onchange="item_change(this);" required>
			   <option value="">Select Item</option>		
			</select>
			<span style="color:red">{{ $errors->first('item_id') }} </span>
		</td>
		<td><input type="text" id="new_qty" name="quantity[]" placeholder="Quantity" class="quantity" required>
			<span style="color:red">{{ $errors->first('quantity') }} </span>
		</td>
		
		<td><input type="button" class="add btn-adds" onclick="" value="">
		</td>
		</tr>
		@else
		@foreach(old('brand_id') as $va1=>$key1)
		<tr class="entry">
		<td><select id="new_brand" name="brand_id[]" class="brand-creation brand-create" placeholder="Brand Name" onchange="brand_change(this);">
			 <option value="">Select Brand</option>
		...
		<td>
			 <?php $items =  DB::table('items')->where('product_name',old('brand_id')[$va1])->get();  ?>
			<select id="new_item"  name="item_id[]" class="brand-creation item-create" placeholder="Item Name" onchange="item_change(this);">
				<option>Select Item</option>
				@foreach($items as $va2=>$key2)			   		
				<option value="{{ $key2->id }}"@if(old('item_id')[$va1]== $key2->id) selected @endif >{{ $key2->type }}</option>
				@endforeach
			</select>
			<span style="color:red">{{ $errors->first('item_id') }} </span>
		</td>
		...
		<div class="form-footer">
			...
			<a class="" href="{{ URL::to('purchase') }}">Cancel</a>
